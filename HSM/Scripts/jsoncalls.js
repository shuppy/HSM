...
var membersnapshot = function (id) {
    $('#memdetails').html = "Loading details...";
    if (xhr != null) xhr.abort();
    xhr = $.ajax({
        type: "POST",
        url: "/members/memsnapshot",
        data: {
            id: id
        },
        cache: false,
        success: function (data) {
            $("#memdetails").html('');
            $("#memdetails").html(data.details);
        },
        error: function () {
            $("#memdetails").html('An error occured loading details, please check selected value.');
        }
    });
}